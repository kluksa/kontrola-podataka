...
def konvert_frejm(frejm=None, stupci=None, func=None):
    """
    #TODO!
    konvert stupca frejma u drugu mjernu jedinicu

    input:
    -frejm --> datafrejm
    -stupci --> lista stringova (imena stupaca)
    -func --> funkcija za konverziju

    output:
    konvertirani datafrejm
    """
    df = frejm.copy()
    for stupac in stupci:
        col = df[stupac].apply(func)
        df[stupac] = col
    return df

class KonverterJedinice(object):
    """
    #TODO!
    Klasa definira funkcije pretvorbe izmedju mjernih jedinica...

    -postavi trenutnu trenutnu mjernu jedinicu i konv. volumen
    -metoda get_convert_function(x) vraca funkciju za konverziju
    """
    def __init__(self):
        self.moguceJedinice = ['ppm', 'ppb', 'mg/m3', 'ug/m3']
        self.treutnaJedinica = 'mg/m3'
        self.konVolumen = 1.0

        self.konverter_map = {
            'ppm':{
                'ppm':self.no_conversion,
                'ppb':self.ppm_to_ppb,
                'mg/m3':self.ppm_to_mgm3,
                'ug/m3':self.ppm_to_ugm3
                },
            'ppb':{
                'ppm':self.ppb_to_ppm,
                'ppb':self.no_conversion,
                'mg/m3':self.ppb_to_mgm3,
                'ug/m3':self.ppb_to_ugm3
                },
            'mg/m3':{
                'ppm':self.mgm3_to_ppm,
                'ppb':self.mgm3_to_ppb,
                'mg/m3':self.no_conversion,
                'ug/m3':self.mgm3_to_ugm3
                },
            'ug/m3':{
                'ppm':self.ugm3_to_ppm,
                'ppb':self.ugm3_to_ppb,
                'mg/m3':self.ugm3_to_mgm3,
                'ug/m3':self.no_conversion
                }
            }

    def set_trenutnaJedinica(self, x):
        self.treutnaJedinica = x

    def get_trenutnaJedinica(self):
        return self.trenutnaJedinica

    def set_konVolumen(self, x):
        self.konVolumen = x

    def get_konVolumen(self):
        return self.konVolumen

    def get_convert_function(self, x):
        """
        vrati funkciju za konvert
        """
        method = self.konverter_map[self.treutnaJedinica][x]
        return method

    #conversion functions
    def no_conversion(self, x):
        return x

    def ppm_to_ppb(self, x):
        return 1000 * x

    def ppm_to_mgm3(self, x):
        return x * self.konVolumen

    def ppm_to_ugm3(self, x):
        return x * self.konVolumen *1000

    def ppb_to_ppm(self, x):
        return x / 1000

    def ppb_to_mgm3(self, x):
        return (x / self.konVolumen) * 1000

    def ppb_to_ugm3(self, x):
        return (x / self.konVolumen)

    def mgm3_to_ppm(self, x):
        return x / self.konVolumen

    def mgm3_to_ppb(self, x):
        return (x * self.konVolumen) / 1000

    def mgm3_to_ugm3(self, x):
        return x * 1000

    def ugm3_to_ppm(self, x):
        return (x / self.konVolumen) / 1000

    def ugm3_to_ppb(self, x):
        return (x * self.konVolumen)

    def ugm3_to_mgm3(self, x):
        return x / 1000
