...
#                names = frejmovi[pName].columns.values
#                names[0] = u'koncentracija'
#                names[1] = u'status'
#                names[2] = u'flag'
#                frejmovi[pName].columns = names
    ...
    #writer metoda
    def save_work(self,frames,filename):
        """
...
if __name__ == "__main__":
    """
    test konzistencije podataka nakon ucitavanja spremljenog.
    """
    data = WlReader().citaj('pj.csv')
    print('stupci unutar frejmova, load input csv-a:\n')
    for key in data:
        print(data[key].columns.values)
    WlReader().save_work(data,'test_save_work.csv')
    data1 = WlReader().load_work('test_save_work.csv')
    print('stupci unutar frejmova, load spremljenog csv-a:\n')
    for key in data:
        print(data1[key].columns.values)    
    #test_frames(data,data1)
    print('stupci unutar frejmova, prethodno ucitanog p.csv:\n')
    for key in data:
        print(data[key].columns.values)
    """
    cudno ponasanje... unutar frejmova data1 mjenjaju se nazivi stupaca..????
    """
